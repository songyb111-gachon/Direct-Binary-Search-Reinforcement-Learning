...
class BinaryHologramEnv(gym.Env):
    def __init__(self, target_function, trainloader, max_steps=10000, T_PSNR=30, T_steps=1, T_PSNR_DIFF=1/4, num_samples=10000, reference_point=0.3):
    ...
    def reset(self, seed=None, options=None, z=2e-3):
        ...
        if positive_psnr_sum < self.reference_point:
            print(f"\033[91m[WARNING] positive_psnr_sum ({positive_psnr_sum:.6f}) is below reference_point ({self.reference_point:.6f}). Retrying with a new dataset sample.\033[0m")
            ...
            # 1만 개 픽셀 플립 후 PSNR 변화량 순위화 및 양수 변화량 총합 계산
            rw_start_time = time.time()
            self.psnr_change_list, self.importance_ranks, positive_psnr_sum = self._calculate_pixel_importance(binary,
                                                                                                               z)
            data_processing_time = time.time() - rw_start_time
            print(
                f"\nTime taken for psnr_change_list: {data_processing_time:.2f} seconds"
            ...
            self.T_PSNR_DIFF = self.T_PSNR_DIFF_o * C
            print(f"\033[94m[Dynamic Threshold] T_PSNR_DIFF set to: {self.T_PSNR_DIFF:.6f}\033[0m")
