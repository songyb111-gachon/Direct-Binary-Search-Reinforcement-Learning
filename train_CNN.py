...
class MultiInputCNN(BaseFeaturesExtractor):
    def __init__(self, observation_space: spaces.Dict, features_dim: int = 256):
        super(MultiInputCNN, self).__init__(observation_space, features_dim)

        CH, IPS = 8, 256

        # ✅ 1. 공통 CNN (state_record, state, pre_model)
        self.shared_cnn = nn.Sequential(
            ...
            nn.ReLU(),
            nn.MaxPool2d(2, 2),
            nn.Conv2d(128, 128, kernel_size=3, stride=1, padding=1),  # ✅ 추가
            nn.ReLU(),
            nn.MaxPool2d(2, 2),  # ✅ 추가 (크기 줄이기)
            nn.Flatten()
        )

        # ✅ 2. 개별 CNN (recon_image, target_image)
        self.single_cnn = nn.Sequential(
        ...
        # CNN 출력 크기 계산
        cnn_out_dim = 128 * (IPS // 16) * (IPS // 16)  # ✅ 기존보다 크기 축소됨
        total_input_dim = (cnn_out_dim * 3) + (cnn_out_dim * 2)
    ...
    def forward(self, observations):
        state_record_feat = self.shared_cnn(observations["state_record"].float().squeeze(1))
        state_feat = self.shared_cnn(observations["state"].float().squeeze(1))
        pre_model_feat = self.shared_cnn(observations["pre_model"].float().squeeze(1))

        recon_image_feat = self.single_cnn(observations["recon_image"].float().squeeze(1))
        target_image_feat = self.single_cnn(observations["target_image"].float().squeeze(1))

        combined_feat = torch.cat([state_record_feat, state_feat, pre_model_feat, recon_image_feat, target_image_feat], dim=1)

        return self.final_fc(combined_feat)
...
if resume_training and os.path.exists(ppo_model_path):
    ...
    ppo_model = PPO(
        ...
        policy_kwargs=dict(
            net_arch=[dict(pi=[128, 128], vf=[128, 64, 1])],  # Critic 가볍게
            features_extractor_class=MultiInputCNN,
            features_extractor_kwargs=dict(features_dim=256),  # 기존 512 → 256
        ),
    )
